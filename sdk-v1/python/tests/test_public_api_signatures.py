...
EXPECTED_TYPES_EXPORTS = list(public_types.__all__)
...
def test_new_public_rpc_methods_have_sync_and_async_mirrors() -> None:
    """Every new Codex/Thread RPC should exist on both sync and async surfaces."""
    codex_methods = [
        "thread_delete",
        "thread_loaded_list",
        "thread_section_list",
    ...
    missing = [
        name for name in codex_methods if not hasattr(Codex, name) or not hasattr(AsyncCodex, name)
    ] + [
        name
        for name in thread_methods
        if not hasattr(Thread, name) or not hasattr(AsyncThread, name)
    ]
    assert missing == []
    assert hasattr(TurnHandle, "settings_update")
    assert hasattr(AsyncTurnHandle, "settings_update")
    assert hasattr(Codex, "experimental")
    assert hasattr(AsyncCodex, "experimental")
